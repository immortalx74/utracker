if (application_state == PLAY_PATTERN)
{
	...
	future_play = std::async(std::launch::async, PlayPattern, module, fsystem, channel,sound, channelgroup, pattern_start, pattern_end, tracks);
}
...
if (application_state == PLAY_MODULE)
{
	...
	future_play = std::async(std::launch::async, PlayModule, module, fsystem, channel,sound, channelgroup, 0, end, tracks);
}
