if (application_state == PLAY_PATTERN)
{
	...
	future_play = std::async(std::launch::async, PlayPattern, fsystem, pattern_start, pattern_end, tracks);
...
if (application_state == PLAY_MODULE)
{
	...
	future_play = std::async(std::launch::async, PlayModule, fsystem, 0, end, tracks);
}
